...
static void file_close(struct file_handle_w3m *handle);
static int file_read(struct file_handle_w3m *handle, char *buf, int len);
...
newFileStream(FILE * f, void (*closep) ())
{
    ...
    stream->file.handle = New(struct file_handle_w3m);
...
static void
file_close(struct file_handle_w3m *handle)
{
    handle->close(handle->f);
}

static int
file_read(struct file_handle_w3m *handle, char *buf, int len)
{
